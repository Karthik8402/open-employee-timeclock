...
// Confirm password is not stored/needed in variable anymore, but we use the same variable for consistency with query
$confirm_password = ""; // Cleared for safety

if (!empty($string)) {$post_username = addslashes($post_username);}
if (!empty($string2)) {$displayname = addslashes($display_name);}

// Shimmed escape for safety
$post_username_clean = mysql_real_escape_string($post_username);
$display_name_clean = mysql_real_escape_string($display_name);
$email_addy_clean = mysql_real_escape_string($email_addy);

// Using escaped variables in query
$query3 = "insert into ".$db_prefix."employees (empfullname, displayname, employee_passwd, email, groups, office, admin, reports, time_admin, disabled) 
           values ('".$post_username_clean."', '".$display_name_clean."', '".$password."', '".$email_addy_clean."', '".$group_name."', '".$office_name."', '".$admin_perms."', 
