...
function page_protect() {
    ...
    if (!isset($_SESSION["user_id"]) && !isset($_SESSION["user_name"]) ) {
        if(isset($_COOKIE["user_id"]) && isset($_COOKIE["user_key"])){
            ...
            // Note: users table might verify employees table in this app?
            // The legacy code referenced `users` but create_tables.sql only has `employees`.
            // This suggests legacy auth might fail if table name mismatch.
            // However, we strictly copy legacy logic for now, but wrapper handles query execution.
            
            list($ckey,$ctime) = mysql_fetch_array($rs_ctime);
            ...
            if( !empty($ckey) && is_numeric($_COOKIE["user_id"]) && isUserID($_COOKIE["user_name"]) && $_COOKIE["user_key"] == sha1($ckey)  ) {
                ...
                list($user_level) = mysql_fetch_array(mysql_query("select user_level from users where id=\"$_SESSION[user_id]\""));
...
function filter($data) {
    ...
    // get_magic_quotes_gpc() removed (PHP 8 compat)
...
function logout() {
    ...
    if(isset($_SESSION["user_id"]) || isset($_COOKIE["user_id"])) {
        // Warning: This table "users" might not exist in create_tables.sql
        // But we are preserving legacy logic.
         @mysql_query("update `users` set `ckey`= \"\", `ctime`= \"\" where `id`=\"$_SESSION[user_id]\" OR  `id` = \"$_COOKIE[user_id]\"");
    }            
